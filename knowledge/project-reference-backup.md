> **快照说明**
>
> 此文件是 2026-04-23 第二轮瘦身**前**的项目参考快照，合并自当时的：
> - `CLAUDE.md`（71 行，第一轮瘦身后的状态）
> - `knowledge/coding-standards.md`（115 行，第一轮从 CLAUDE.md 下沉出来的实现规范）
>
> 用途：供人工阅读和备份，**不被 agent 加载**。生产规则请以实时的 `CLAUDE.md` 和 `knowledge/` 下的文件为准。
>
> 如果后续 `CLAUDE.md` 或 `coding-standards.md` 有更新，本文件不会自动同步。

---

# 第一部分：CLAUDE.md 快照（71 行）
...
详细实现参见 `knowledge/project-patterns.md`。
...
# 第二部分：knowledge/coding-standards.md 快照（115 行）
